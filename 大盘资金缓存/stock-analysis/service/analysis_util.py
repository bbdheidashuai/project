...
import numpy as np  # 导入数值计算核心库numpy，简写为np


def calc_alpha(annualized_returns, benchmark_annualized_returns, beta, rf):
    """
    annualized_returns：策略年化收益率
    benchmark_annualized_returns：基准年化收益率
    beta：策略贝塔值（由 calc_beta 计算）
    rf：无风险利率（如 0.04 代表 4%）
...
def calc_beta(assest_profit, benchmark_profit):
    ...
    assest_profit：策略每日收益列表
    benchmark_profit：基准每日收益列表
    ...
    # 第二步：计算协方差矩阵（numpy.cov返回二维矩阵）
    calc_cov = np.cov(assest_profit, benchmark_profit)
    # 第三步：协方差矩阵[0,1]是策略与基准的协方差，[1,1]是基准的方差
...
def calc_annualized_returns(total_returns, days):
    """
    策略年化收益
    total_returns：策略累计收益（如 0.3 代表 30%）
    days：策略运行天数
    Total Annualized Returns = Rp = ((1+P)^250/n − 1) ∗ 100%
    P=策略收益
    n=策略执行天数
    """
    # np.power：计算(1+总收益)的(250/天数)次方，再减1
...
def calc_volatility(daily_yields, days):
    ...
    # 第二步：计算每个交易日收益与均值的差的平方和，再乘以年化系数，最后开平方
